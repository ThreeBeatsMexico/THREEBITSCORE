...
namespace ThreeBits.Shared
{
    public class Utils
    {
        ...
        public void writeLogFile(LogFileBE item)
        {

            string path = _configuration["TBSettings:FolderLogs"]; 
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
            string filepath = path + item.psServiceName + "/" + item.psServiceName + "Log_" + DateTime.Now.Date.ToShortDateString().Replace('/', '_') + ".txt";
            if (!File.Exists(filepath))
            {
                // Create a file to write to.   
                using (StreamWriter sw = File.CreateText(filepath))
                {
                    sw.WriteLine(item.psMessage);
                }
            }
            else
            {
