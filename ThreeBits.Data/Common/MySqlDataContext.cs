...
namespace ThreeBits.Data.Common
{
    public class MySqlDataContext
    {
        ...
        protected string _MySqlconnectionString;
        protected MySqlConnection _MysqlConnection = null;
        ...
        public bool ExecuteNonQuery(ref MySqlCommand command, out int rowsAffected, out string dbException, int commandTimeout = COMMANDTIMEOUT)
        {
            rowsAffected = 0;
            try
            {
        ...
        public bool ExecuteScalar(ref MySqlCommand command, out dynamic scalar, out string dbException, int commandTimeout = COMMANDTIMEOUT)
        {
            scalar = null;
            try
            {
        ...
        public bool ExecuteReader(ref MySqlCommand command, out DataTable reader, out string dbException, CommandBehavior behavior = CommandBehavior.Default, string TableName = "tabla", int commandTimeout = COMMANDTIMEOUT)
        {
            reader = null;
            try
            {
                if (Connect(out dbException))
                {
                    command.Connection = _MysqlConnection;
                    command.CommandTimeout = commandTimeout;
                    MySqlDataReader DataReader = command.ExecuteReader(behavior);
